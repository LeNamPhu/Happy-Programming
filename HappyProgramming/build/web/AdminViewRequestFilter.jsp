<%-- 
    Document   : AdminViewRequestFilter
    Created on : Jul 5, 2022, 11:37:55 AM
    Author     : DELL
--%>
    ...
    <body>
        ...
        <div class="body">
            <%  String spageid = request.getParameter("page");
                ...
                Date from = Date.valueOf("2022-01-01");
                ...
                ArrayList<Request> list = RequestDAO.filterRequestPagination(keyword, status, from, to, pageid, total);
                ArrayList<Request> allRequest = RequestDAO.filterRequest(keyword, status, from, to);
                int count = 1;
                int pages = 0;
            ...
            <form action="MainController" method="post">
                <div class="input-group " style="margin-top: 2%;margin-left: 2%">
                    ...
                    <span style="margin-left: 2%">
                        From <input type="date" name="from" <%if(!request.getParameter("from").isEmpty()){%> value="${param.from}"<%} else{%> value="2022-01-01"<%}%> style="height:40px ">
                        to <input type="date" name="to" <%if(!request.getParameter("to").isEmpty()){%> value="${param.to}" <%} else{%> value="<%=new Date(System.currentTimeMillis())%>" <%}%> style="height:40px ">
                    </span>
            ...
            <nav aria-label="Page navigation example" style="margin-top:100px;margin-left: 450px ">
                <ul class="pagination">
                    ...
                    <li class="page-item <%if (Integer.parseInt(request.getParameter("page")) == 1) {%>disabled<%}%>">
                        <a class="page-link" href="AdminViewRequestFilter.jsp?page=<%=Integer.parseInt(request.getParameter("page")) - 1%>&txtsearch=<%=request.getParameter("txtsearch")%>" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                            <span class="sr-only">Previous</span>
                        </a>
                    </li>
                    <%for (int i = 1; i <= pages; i++) {%>
                    <li class="page-item"><a class="page-link" href="AdminViewRequestFilter.jsp?page=<%=i%>&txtsearch=<%=request.getParameter("txtsearch")%>"><%=i%></a></li>
                        <%  }
                        %>

                    <li class="page-item <%if (Integer.parseInt(request.getParameter("page")) == pages) {%>disabled<%}%>">
                        <a class="page-link" href="AdminViewRequestFilter.jsp?page=<%=Integer.parseInt(request.getParameter("page")) + 1%>&txtsearch=<%=request.getParameter("txtsearch")%>" aria-label="Next">
